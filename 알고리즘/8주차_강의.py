...
2) 파일 입출력
3) 랜덤 함수 불러오기
4) "평균 시간 복잡도가 ~인 알고리즘으로 문제를 해결하라." 형식으로 문제가 나올 수도 있음.
"""
#탐색 알고리즘
#순차 검색
...
def SelSort(ary):
    for i in range(len(ary) - 1, 0, -1):
        ...
        for j in range(i + 1):
            if ary[j] > ary[max]:
                max = j
        ary[max], ary[j] = ary[j], ary[max]
    return ary


#버블 정렬: 평균 시간 복잡도 = O(n^2)
def BubSort(ary):
