def print_array(ary, seq_ref, seq_target):
    (row, col) = (len(seq_target), len(seq_ref))
...
def summatrix(seq_ref, seq_target):
    ...
    array = [[1 if seq_ref[co] is seq_target[ro] else 0 for co in range(col)] for ro in range(row)]  # 참조 서열 길이 x 대조 서열 길이 크기의 2차원 배열 생성.  동시에 두 서열을 비교하여 서로 같은 문자이면 1, 다르면 0을 추가함.
...
def alignseq(array, seq_ref, seq_target):
    ...
    ary_mod = modarray(array)  # 참조하는 index 범위를 맞추기 위해 행과 열을 2씩 늘림.(결과에는 영향을 주지 않음)
